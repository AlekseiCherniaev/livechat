...
class TestUsersAPI:
    async def test_register_success(self, async_client: AsyncClient):
        ...
        response = await async_client.post("/api/users/register-user", json=payload)
        assert response.status_code == 200

    async def test_register_user_already_exists(self, async_client: AsyncClient):
        payload = {"username": "bob", "password": "secret"}
        await async_client.post("/api/users/register-user", json=payload)
        response = await async_client.post("/api/users/register-user", json=payload)
    ...
    async def test_login_success(self, async_client: AsyncClient):
        ...
        response = await async_client.post("/api/users/login-user", json=payload)
        assert response.status_code == 200
    ...
    async def test_login_invalid_credentials(self, async_client: AsyncClient):
        ...
        response = await async_client.post("/api/users/login-user", json=wrong_payload)
        assert response.status_code == 401

    async def test_me_success(self, async_client: AsyncClient):
        payload = {"username": "eve", "password": "secret"}
        await async_client.post("/api/users/register-user", json=payload)
        login_resp = await async_client.post("/api/users/login-user", json=payload)

        cookies = {"session_id": login_resp.cookies["session_id"]}
        resp = await async_client.get("/api/users/get-me", cookies=cookies)
    ...
    async def test_me_invalid_session(self, async_client: AsyncClient):
        resp = await async_client.get(
            "/api/users/get-me", cookies={"session_id": "invalid"}
        )
    ...
    async def test_full_flow(self, async_client: AsyncClient):
        ...
        me_resp = await async_client.get(
            "/api/users/get-me", cookies={"session_id": session_id}
        )
        assert me_resp.status_code == 200
        assert me_resp.json()["username"] == "grace"

        logout_resp = await async_client.post(
            "/api/users/logout-user", cookies={"session_id": session_id}
        )
